...
int hciSetParams (int socket, int itv, int timeOut) {
    ...
    params.chan_map = 7;
    req.ogf = 0x08; // LE controller commands
    req.ocf = 0x0006; // set LE advertising parameters
    req.cparam = &params;
    req.rparam = &status;
    req.clen = 15;
    req.rlen = 1;
...
int sendPacket (char* bArr) {
    ...
    /*
    status = hciSetParams(socket, 0x0020, 1000);
    if (status < 0) {
        fprintf(stderr, "[E] Failed to set advertising parameters! %d\n", status);
        return 1;
    }*/
    printf("hci_send_cmd ");
    for (int i = 0; i < 32; ++i) {
        printf("%02x", (uint8_t)bArr[i]);
    }
    printf("\n");
    status = hci_send_cmd(socket, 0x08 /* LE controller commands */ , 0x0008 /* le set advertising data */, 32, bArr);
    if (status < 0) {
        fprintf(stderr, "[E] Failed to set advertising data! %d\n", status);
        return 1;
    }
...
// register:
// 1f0201011b03f9084913f069254e3151baaee5505993fa4971a3f45568f6dd00
// 0x1f 0x02 0x01 0x01 0x1b 0x03 0xf9 0x08 0x49 0x13 0xf0 0x69 0x25 0x4e 0x31 0x51 0xba 0xae 0xe5 0x50 0x59 0x93 0xfa 0x49 0x71 0xa3 0xf4 0x55 0x68 0xf6 0xdd 0x00 
//
// 0x10 0x00 0xea 0xd5 0x96 0x0a 0x92 0x64 0x20 0xe6 0x2b 0xad 0x79 0xbb 0xf1 0x9f 0x2a 0x0e 0xb1 0xfb 0x0c 0xa9 0x1e 0x37


// ManufacturerData.Key 0x5570
// ManufacturerData.Value 0x18 0x87 0x52 0xb6 0x5f 0x2b 0x5e 0x00 0xfc 0x31 0x51 0x10 0xab 0x9b 0x08 0x24 0xcb 0x00 0xfc 0x4e 0x9c 0x87 0xea 0x81 0x4f 0x60 0x57
// 0x18 0x87 0x52 0xb6 0x5f 0x2b 0x5e 0x00 0xfc 0x31 0x51 0xd0 0xab 0x9b 0x08 0x24 0xcb 0x9c 0xfc 0x28 0xfa 0xd3 0x8c 0xbc 0xee 0x60 0x57

// Service.000008f0-0000-1000-8000-00805f9b34fb:
// 0x10 0x00 0xea 0xd5 0x96 0x0a 0x92 0x64 0x20 0xe6 0x2b 0xad 0x79 0xbb 0xf1 0x9f 0x2a 0x0e 0xb1 0xfb 0x0c 0xa9 0x1e 0x37


// on 
// 0x18 0x87 0x52 0xb6 0x5f 0x2b 0x5e 0x00 0xfc 0x31 0x51 0xd0 0xab 0x9b 0x08 0x24 0xcb 0x4b 0xfc 0x2a 0xf8 0x89 0x8e 0x38 0xd7 0xff 0xff 
